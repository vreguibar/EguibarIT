function Write-CustomVerbose {
    ...
    <#
        .SYNOPSIS
            Mimics Write-Verbose but with optional logging to the Windows Event Log.

        .DESCRIPTION
            This function writes verbose messages to the console and, if instructed, to the specified Windows Event Log.
        ...
        .EXAMPLE
            # Log a verbose message and also create a Windows Event Log entry with predefined event info
            Write-CustomVerbose -CreateWindowsEvent -EventInfo ([EventIDs]::SlowPerformance) -Message "Old hardware detected."
        ...
        .EXAMPLE
            ...
            Write-CustomVerbose @Splat

        .EXAMPLE
            # Log a custom event with specific event details
            Write-CustomVerbose -CreateWindowsEvent -EventId 5001 -EventName "CustomEvent" -EventCategory SystemHealth
    ...
    Process {
        if ($PSCmdlet.ShouldProcess("Writing verbose log: $Message")) {
            ...
            Write-Verbose -Message $Message -Verbose:$PSBoundParameters['Verbose']
        }
    } #end Process
