function New-WsusObjects {
    <#
        .SYNOPSIS
            Creates Windows Server Update Services (WSUS) objects and delegations.

        .DESCRIPTION
            Creates the WSUS objects used to manage this organisation by following the defined
            Delegation Model. Key features:
            - Creates WSUS-specific security groups
            - Configures WSUS update permissions
            - Delegates WSUS administration rights
        ...
        .EXAMPLE
            New-WsusObjects -ConfigXMLFile 'C:\PsScripts\Config.xml'

            Creates WSUS objects using the specified configuration file.

        .EXAMPLE
            New-WsusObjects -ConfigXMLFile 'C:\PsScripts\Config.xml' -Verbose

            Creates WSUS objects with verbose output.

        .EXAMPLE
            New-WsusObjects -ConfigXMLFile 'C:\PsScripts\Config.xml' -WhatIf

            Shows what would happen when creating WSUS objects.
        ...
        .LINK
            https://github.com/vreguibar/EguibarIT/blob/main/Public/New-WsusObjects.ps1
